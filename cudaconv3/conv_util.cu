...
void sum(NVMatrix& src, int axis, NVMatrix& target) {
  src._aggregate(axis, target, NVMatrixAggs::Sum(), NVMatrixOps::Identity(), NVMatrixBinaryOps::Second(), src.getDefaultStream());
}
...
__global__ void kCNorm_manyfilter(float* imgs, float* meanDiffs, float* denoms, float* target, const int imageY, const int imageX,
    ...
    const int pxIdxX = blockIdx.z % imageX;
    const int pxIdxY = blockIdx.z / imageX;
...
__global__ void kFCNorm(cudaTextureObject_t imgs, cudaTextureObject_t meanDiffs, float* target, const int imageY, const int imageX,
...
__global__ void kFRNormUndo(cudaTextureObject_t outGrads, cudaTextureObject_t denoms, cudaTextureObject_t inputs, cudaTextureObject_t acts,
                            float* target, const int imageY, const int imageX, const int numFilters, const int numImages, const int sizeF, const float powScale,
                            const float scaleTargets, const float scaleOutputs) {
    const int numImgBlocks = DIVUP(numImages,B_X*imgsPerThread);
    const int numFilterBlocks = numFilters/B_Y;
...
__global__ void kFRNormUndo2(cudaTextureObject_t outGrads, cudaTextureObject_t inputs, cudaTextureObject_t acts, float* target, const int imageY, const int imageX, const int numFilters,
    ...
    if (sizeX >= 6 && numFilters % 4 == 0) {
        ...
        if (checkCaseBounds) {
            ...
            kCNorm2<8, 8, 4, true><<<blocks, threads, 0, stream>>>(images.getDevData(), meanDiffs.getDevData(), denoms.getDevData(), target.getDevData(),
                                                           imageY, imageX, numFilters, numImages, sizeX, addScale, powScale, minDiv);
        } else {
            ...
            if (numFilters == 1) {
                if (checkCaseBounds) {
                    ...
                    kCNorm_fewfilter<1, 1, true><<<blocks, threads, 0, stream>>>(images.getDevData(), meanDiffs.getDevData(), denoms.getDevData(), target.getDevData(),
                                                                      imageY, imageX, numImages, sizeX, addScale, powScale, minDiv);
                } else {
    ...
    if (blocked) {
        if (checkCaseBounds) {
            ...
            kFCNorm<4, 32, 4, true, true><<<blocks, threads, 0, stream>>>(images.getTextureObject(), meanDiffs.getTextureObject(), target.getDevData(),
                                                                imageY, imageX, numFilters, numImages, sizeF, addScale, powScale, minDiv);
        } else {
        ...
        if (scaleTargets == 0 && scaleOutput == 1) {
            if (checkCaseBounds) {
                ...
                kFRNormUndo2<4, 32, 4, false, true, true><<<blocks2, threads2, 0, stream>>>(outGrads.getTextureObject(), inputs.getTextureObject(), acts.getTextureObject(),
                                                                        target.getDevData(), imageY, imageX, numFilters, numImages, sizeF, addScale, powScale, minDiv,
                                                                        scaleTargets, scaleOutput);
            } else {
