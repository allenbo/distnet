...
if sys.argv[1] == 'single':
  ...
  print  'rnorm cross map', time.time() - start

  output = output.reshape(out_shape)
  denom = denom.reshape(out_shape)
  with open('rnormcrossmap-output-single', 'w') as f:
  ...
  arr.rnormcrossmap(input,denom, output,
      96, size, image_y, scale, pow, False)
  print 'rnorm cross map', time.time() - start
  output.gather()
  denom.gather()
  if rank == 0:
    with open('rnormcrossmap-output-multi', 'w') as f:
